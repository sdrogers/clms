...
class Sample_Dataset(object):
    ...
    Attributes:
        ...
        ms2_intensity_noise_sd: standard deviation of intensity noise in ms2 peaks
    """
    def __init__(self, file_location, rt=None, n_ms1=None, n_ms2=None, ms2_mz_noise_sd=0, ms2_intensity_noise_sd=0, alpha=math.inf, noise_peaks=False, noise_peaks_intensity=None, noise_peaks_intensity_sd=None, noise_peaks_mz_sd=None, rt_length=None, n_noise_peaks=None):
        ...
        if noise_peaks == True:
            ms1_noise_peak = ps.sample_noise_peak(ms_level = 1, noise_peaks_intensity = noise_peaks_intensity, noise_peaks_intensity_sd = noise_peaks_intensity_sd, noise_peaks_mz_sd = noise_peaks_mz_sd, noise_peaks_rt_length = noise_peaks_rt_length, n_noise_peaks = n_noise_peaks)
            if rt != None:
        ...
        if extra_bins > 0:
            for j in range(extra_bins):
                for i in range(64*(2**j)):
                    locations_internal[n_locations_internal+j].append((bin_walls_extra[int(0  + i*((2**extra_bins)/(2**j)))], bin_walls_extra[int(((2**extra_bins)/(2**j))/2  + i*((2**extra_bins)/(2**j)))]))
